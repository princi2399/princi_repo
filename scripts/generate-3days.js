...
/**
 * Generate realistic Overwatch test alerts spanning the last 3 days
 * and POST them to a target webhook URL.
 *
 *   node scripts/generate-3days.js                                # localhost
 *   TARGET=https://princi-repo.onrender.com node scripts/generate-3days.js
 */
const TARGET = (process.env.TARGET || 'http://localhost:3001').replace(/\/$/, '');
const WEBHOOK = `${TARGET}/webhook/overwatch`;
...
function buildAlert(idx) {
  ...
  const startedAt = new Date(receivedAt.getTime() - (5 + Math.floor(Math.random() * 120)) * 60000);
  ...
  return {
    ...
    notification_triggered_at: receivedAt.toISOString(),
...
async function main() {
  console.log(`Posting ${COUNT} alerts across last ${DAYS} day(s) → ${WEBHOOK}`);
  let ok = 0, fail = 0;
  for (let i = 0; i < COUNT; i++) {
    const alert = buildAlert(i);
    try {
      ...
      if ((i + 1) % 10 === 0) console.log(`  ${i + 1}/${COUNT} sent…`);
    } catch (e) {
      fail++;
